...
from FlorinaRobot import (DEV_USERS, OWNER_ID, DRAGONS, DEMONS, TIGERS, WOLVES,
                          INFOPIC, dispatcher, sw)
from FlorinaRobot.__main__ import STATS, TOKEN, USER_INFO
import FlorinaRobot.modules.sql.userinfo_sql as sql
from FlorinaRobot.modules.disable import DisableAbleCommandHandler
from FlorinaRobot.modules.sql.global_bans_sql import is_user_gbanned
from FlorinaRobot.modules.sql.afk_sql import is_afk, check_afk_status
from FlorinaRobot.modules.sql.users_sql import get_user_num_chats
from FlorinaRobot.modules.helper_funcs.chat_status import sudo_plus
from FlorinaRobot.modules.helper_funcs.extraction import extract_user
from FlorinaRobot import telethn as EmiliaTelethonClient
...
def info(update: Update, context: CallbackContext):
    ...
    if user.id == OWNER_ID:
        text += "\n\nThe Ranking of The Person is Owner of Alexia"
        disaster_level_present = True
    elif user.id in DEV_USERS:
        text += "\n\nThis Person is a Developer of Alexia"
        disaster_level_present = True
    elif user.id in DRAGONS:
        text += "\n\nThis Person is a Sudo-User of Alexia"
        disaster_level_present = True
    elif user.id in DEMONS:
        text += "\n\nThis Person is a Support User of Alexia"
        disaster_level_present = True
    elif user.id in TIGERS:
        text += "\n\nThis User is Immune to all Restrictions over Alexia"
        disaster_level_present = True
...
def __user_info__(user_id):
    ...
    result = ""
    if me:
        result += f"<b>Self Satisfaction:</b>\n{me}\n"
    if bio:
        result += f"<b>What others Speak Of:</b>\n{bio}\n"
    result = result.strip("\n")
    return result
