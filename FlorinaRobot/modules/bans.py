...
import FlorinaRobot.modules.sql.users_sql as sql
...
def ban(update: Update, context: CallbackContext) -> str:
    ...
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        if user_id == OWNER_ID:
            message.reply_text("Trying to put me against a Owner huh?")
        elif user_id in DEV_USERS:
            message.reply_text("I can't act against our Dev.")
            ...
            message.reply_text(
                "Bring an order from Captain to fight a VexanaFanclub servant."
            )
        elif user_id in TIGERS:
            message.reply_text("Bring an order from Soldier to fight a Tigers servant.")
...
KICKME_HANDLER = DisableAbleCommandHandler(
    ["kickme", "punchme"], punchme, filters=Filters.chat_type.groups,
)
